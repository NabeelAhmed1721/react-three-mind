...
import { Controller as FaceTargetController } from "../lib/controller";
...
const ARProvider = forwardRef(
  (
    ...
    // recalculate estimator on window change
    /* Lot of room for optimization:
      - Don't just recreate the entire class (could be expensive)
      - Debounce `useWindowSize`
      - etc.
    */
    useEffect(() => {
      if (controllerRef.current && webcamRef.current && webcamRef.current.video) {
        const video = webcamRef.current.video;
        const controller = controllerRef.current;
        controller.recalculateEstimator(video);
